...
class ChooseWorkspaceActionDialog(QDialog):
    CANCEL = 0
    DL_TO_MEM = 1
    OPEN_ON_WEB = 2
    ...
    def check_radio_button(self, choose_download_to_mem, choose_open_on_web):
        ...
        workspace_action = self.FROM_STRING_WORKSPACE_ACTIONS.get(
            param_group.GetString(self.PREF_WORKSPACE_ACTION), self.CANCEL
        )

        if workspace_action == self.DL_TO_MEM:
            choose_download_to_mem.setChecked(True)
        elif workspace_action == self.OPEN_ON_WEB:
            choose_open_on_web.setChecked(True)
        else:
            choose_download_to_mem.setChecked(True)
    ...
    def accept(self):
        ...
        param_group.SetString(
            self.PREF_WORKSPACE_ACTION, self.TO_STRING_DOWNLOAD_ACTIONS[self.answer]
        )
        super().accept()
